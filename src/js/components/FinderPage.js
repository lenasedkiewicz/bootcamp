...
class FinderPage {
  ...
  render(element) {
    ...
    /* find menu container */
    const finderContainer = document.querySelector(
      select.containerOf.finderPage
    );
    /* add element to menu */
  ...
  initActions() {
    ...
    thisFinder.dom.gridContainer = document.querySelector(
      select.containerOf.gridContainer
    );
    ...
    thisFinder.dom.gridContainer.addEventListener('click', function (event) {
      ...
      const cellRight = thisFinder.clickedGrid.includes(
        parseInt(clickedElementId + 1)
      );
      const cellLeft = thisFinder.clickedGrid.includes(
        parseInt(clickedElementId - 1)
      );
      const cellTop = thisFinder.clickedGrid.includes(
        parseInt(clickedElementId - 10)
      );
      const cellBottom = thisFinder.clickedGrid.includes(
        parseInt(clickedElementId + 10)
      );
      ...
      if (clickedElement.getAttribute('class') === classNames.finder.gridItem) {
        ...
        if (thisFinder.clickedGrid.length === 0) {
          thisFinder.clickedGrid.push(
            parseInt(clickedElement.getAttribute('id'))
          );
        ...
        // if there is grid active check for conditions for next active grid
          ...
          if (
            ...
            thisFinder.lastClicked.classList.replace(
              classNames.finder.gridItemLastClicked,
              classNames.finder.gridItemClicked
            );
      ...
      //CONDITIONS FOR REMOVING MARKED CELLS
        ...
        clickedElementId ===
        thisFinder.clickedGrid[thisFinder.clickedGrid.length - 1]
      ) {
        ...
        const clickedId = thisFinder.clickedGrid.indexOf(
          parseInt(clickedElement.getAttribute('id'))
        );
        thisFinder.clickedGrid.splice(clickedId, 1);

        const previousClickedId =
          thisFinder.clickedGrid[thisFinder.clickedGrid.length - 1];
